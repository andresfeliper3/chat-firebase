...
function signOut(){
    ...
    remove(deleteRef);
    deleteRooms(); //borrar las salas abiertas
    location.reload();
}
...
function addUser(data){
    ...
    $li.on('click', function() {
        ...
        //new Chat(room.key, user, "chats", database, ref, push, onChildAdded);
    }); 
...
function newRoom(data){
    ...
    if(data.val().friend == user.uid){
        new Chat(data.key, user, "chats", database, ref, push, onChildAdded);
    }
...
//Borra la salas del usuario que se desconecta
function deleteRooms(){
    for(let roomKey of roomKeys){
        let deleteRef = ref(database, "/rooms/"+roomKey);
        remove(deleteRef);
    }
}